...
function login($email, $password) {
    ...
    $sql = "SELECT u_id, u_email, u_password, u_salt, u_isAdmin FROM user WHERE u_email = :email LIMIT 1";
    if ($row = $pdo->query($sql,array("email"=>$email)))
    {
        if ($row)
        {
            $user_id = $row[0]["u_id"];
            $email = $row[0]["u_email"];
            $db_password = $row[0]["u_password"];
            $salt = $row[0]["u_salt"];
            $isAdmin = $row[0]["u_isAdmin"];
            ...
            if ($db_password == $password) {
                ...
                // $now = time();
                //$brut = $pdo->prepare("INSERT INTO hs_login_attempts(u_id, la_time) VALUES ('$user_id', '$now')");
                //$brut->execute();
...
function signup() {
    ...
    if (isset($_POST['identifiant'], $_POST['prenom'], $_POST['nom'], $_POST['email'], $_POST['password'], $_POST['password2'], $_POST['id_skype'], $_POST['id_minecraft'])) {
        $_SESSION['error'] = "ok";
        ...
        $id_minecraft = $_POST['id_minecraft'];
        ...
        $sql = "SELECT u_id FROM user WHERE u_email = :email LIMIT 1";
        if ($row = $pdo->query($sql,array("email"=>$email))) {
        ...
        $sql = "INSERT INTO user (u_identifiant, u_prenom, u_nom, u_id_skype, u_id_minecraft, u_email, u_password, u_salt, u_dateInscription) VALUES (:identifiant, :prenom, :nom, :id_skype, :id_minecraft, :email, :password, :salt, now())";
        if ($row = $pdo->query($sql,array("identifiant"=>$identifiant, "prenom"=>$prenom,"nom"=>$nom,"id_skype"=>$id_skype,"id_minecraft"=>$id_minecraft,"email"=>$email,"password"=>$password,"salt"=>$random_salt)))
        {
             return true;
        } else {
            $_SESSION['error'] = "Erreur de base de données2";
            return false;
        }
    } else {
        $_SESSION['error'] = "Erreur de base de données3";
        return false; 
    }
}
